...
export const GENRES: GenreConfig[] = [
  ...
  },
  {
    ...
    footnote:
      "ここに表示されていない路線は、現在運行情報が出ていません(平常運転)。対象はJR東日本・東京メトロ・都営地下鉄・東急・京王・西武・東武です。小田急線・京成線は、データ提供元(ODPT)に運行情報がないため対象外です。初期表示は首都圏(東京都・神奈川県・埼玉県・千葉県の1都3県)に絞っています。茨城・栃木・群馬・山梨などにまたがる路線は、主な区間が1都3県内にあるかで分けています。",
  },
  {
    id: "traffic",
    label: "渋滞情報",
